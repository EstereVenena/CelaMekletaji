...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    // Saņem tikai lietotājvārdu un paroli
    $lietotajvards = trim($_POST["lietotajvards"] ?? "");
    $parole = $_POST["parole"] ?? "";

    if ($lietotajvards === "" || $parole === "") {
        $kluda = "Lūdzu, ievadi lietotājvārdu un paroli.";
    } else {

        // Atlasa lietotāju tikai pēc lietotājvārda
        $sql = "
            SELECT 
                u.lietotajs_id,
                u.lietotajvards,
        ...
        if ($rez && $rez->num_rows === 1) {
            ...
            // Pārbauda, vai konts nav īslaicīgi bloķēts
            if (!empty($lietotajs["blokets_lidz"]) && strtotime($lietotajs["blokets_lidz"]) > time()) {
                ...
                // Atiestata neveiksmīgos mēģinājumus
